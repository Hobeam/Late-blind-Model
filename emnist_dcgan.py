...
parser.add_argument('--outf', default='./save_models', help='folder to output images and model checkpoints')
    ...
    # for i, (data, _) in enumerate(dataloader, 0):
        ...
        if i % 100 == 0:
            ...
            vutils.save_image(fake.detach(),
                    '%s/dcgan_emnist_fake_samples_epoch_%03d.png' % (opt.outf, epoch),
                    normalize=True)

    # do checkpointing
torch.save(netG.state_dict(), '%s/emnist_G_best.pth.tar' % (opt.outf))
torch.save(netD.state_dict(), '%s/emnist_D_best.pth.tar' % (opt.outf))